...
You can pull a pre-built docker image from docker hub:
    ...
    docker pull pnwairfire/bluesky:v4.3.51
...
script included in this repo:
    ...
    ./dev/scripts/docker/add-user-to-bluesky-image \
        -i pnwairfire/bluesky:v4.3.51 -n bluesky2
...
Then, to install, for example, v4.3.51, use the following (with sudo if necessary):

    pip3 install --no-binary gdal --extra-index https://pypi.airfire.org/simple bluesky==4.3.51
