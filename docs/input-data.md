...
###### 'activity'
 ...
 - ***'fires' > 'activity' > 'start'*** -- *required if activity is specified* -- can be inferred from other fields
 - ***'fires' > 'activity' > 'end'*** -- *required if activity is specified* --
...
If no activity objects are specified for a fire, but
the fire has 'start' and 'end' keys, a activity object will be created with
...
The following fields may be specified within a fire activity window's "location"
...
###### Other fields
 ...
 - ***'fires' > 'date_time'*** -- *optional* -- if defined, date_time is used to infer activity window and utc offset (if possible), which are used if not already defined for the fire
