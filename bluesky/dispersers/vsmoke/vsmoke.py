...
#         input_file = context.full_path("VSMOKE.IPT")
#         iso_input_file = context.full_path("vsmkgs.ipt")

#         # Define variables for making KML and KMZ files
#         intervals = VSMOKEDispersion.ISOPLETHS
...
#                 self._write_iso_input(emis, intervals, hr, in_var, iso_input_file)
...
#                 self._build_kml(kml_path, in_var, iso_file, intervals, contour_names, contour_colors)
#                 kmz_files.append(kml_path)
#                 my_kmz.add_kml(kml_name, fireLoc, hr)

#                 # Add data to GeoJSON file
#                 if create_json:
#                     self._build_json(json, in_var, iso_file, intervals, contour_names, contour_colors, fireLoc['id'], timezone, hr)
...
#         # Save locations of output files
#         fireInfo['metadata']['kmz_file_name'] = kmz_filename
#         if create_json:
#             fireInfo['metadata']['json_file_name'] = final_json
...
#     def _write_iso_input(self, emissions, intervals, hour, in_var, input_file):
...
#     def _build_isopleths(self, in_var, iso_file, intervals):
...
#     def _build_kml(self, kml_file, in_var, iso_file, intervals, contour_names, contour_colors):
#         """
#         Make a KML file.
...
#         isopleths = self._build_isopleths(in_var, iso_file, intervals)
...
#     def _build_json(self, json, in_var, iso_file, intervals, contour_names, contour_colors, fire_id, timezone, hr):
#         isopleths = self._build_isopleths(in_var, iso_file, intervals)
