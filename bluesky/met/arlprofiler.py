...
class ArlProfiler(object):
    ...
    def _load(self, full_path_profile_txt, first, start, end, utc_offset, lat, lng):
        ...
        self._fill_in_fields(hourly_profiles, first, utc_offset, lat, lng)
    ...
    def _fill_in_fields(self, hourly_profiles, first, utc_offset, lat, lng):
        # The following is from BSF
        tmidday = datetime(first.year, first.month, first.day, 12)
        s = Sun(lat=lat, long=lng)
        sunrise = s.sunrise_hr(tmidday) + utc_offset
        sunset = s.sunset_hr(tmidday) + utc_offset
        ...
        for dt, hourly_profile in hourly_profiles.items():
            hr = (dt - first).total_seconds() / 3600.0
            hourly_profile['lat'] = lat
            hourly_profile['lng'] = lng
            for k in ['pressure', 'TPOT', 'WSPD', 'WDIR', 'WWND', 'TEMP', 'SPHU']:
                hourly_profile[k] = hourly_profile.get(k)
            if not hourly_profile.get('HGTS'):
                hourly_profile['HGTS'] = self.calc_height(hourly_profile['pressure'])
            if not hourly_profile.get('RELH'):
                hourly_profile['RELH'] = self.calc_rh(hourly_profile['pressure'],
                    hourly_profile['SPHU'], hourly_profile['TEMP'])
            hourly_profile['dew_point'] = self.calc_dew_point(
                hourly_profile['RELH'], hourly_profile['TEMP'])
