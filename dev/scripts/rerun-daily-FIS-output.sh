...
if [ $# -lt 4 ]
  then
    echo ""
    echo "Usage: $0 <host_output_root_dir> <init_time> <mount_local_code> <module_1> [optional_module_2] [...]"
    echo ""
    echo "Examples:"
    echo "  $0 $HOME/code/pnwairfire-bluesky/tmp/ 2018062100 TRUE visualization"
    echo "  $0 /data/bluesky-pipeline/output/standard/PNW4km-FS-FIS/ 2018062100 FALSE dispersion visualization"
    echo ""
    exit 1
fi
...
BSP_CMD=$BSP_CMD' -v '$HOST_OUTPUT_ROOT_DIR/$INIT_TIME'/forecast:/data/bluesky-daily/output/PNW4km-FireSpider-FIS/'$INIT_TIME'/'
...
BSP_CMD=$BSP_CMD' -i /data/bluesky-daily/output/PNW4km-FireSpider-FIS/'$INIT_TIME'/output-indented.json'
BSP_CMD=$BSP_CMD' -o /data/bluesky-daily/output/PNW4km-FireSpider-FIS/'$INIT_TIME'/output-rerun.json'
